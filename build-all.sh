...
build_and_tag () {
    ...
    # Build the image
    docker buildx build --build-arg UBUNTU_VERSION=${UBUNTU_VERSION} --build-arg APT_REPOSITORY=${APT_REPOSITORY} --build-arg PYTHON_VERSION=${PYTHON_VERSION} --build-arg NODE_VERSION=${NODE_VERSION} --platform linux/amd64,linux/arm/v8 -t ${PRIMARY_TAG_NAME} -f Dockerfile --push . 2>> ${LOG_FILE}
